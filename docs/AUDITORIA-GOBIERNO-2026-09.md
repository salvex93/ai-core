...
| G16 | PARCIAL. Hecho 2026-09-21: `scripts/quality-gate-checks.js` agrega al gate los checks de archivos residuales (`*.new`, `*.orig`, `*.rej`, tracked o sin ignorar) y de credenciales de alta confianza en el working tree (reusa `ALTA_CONFIANZA` de `patrones-secretos.js`; `tests/` exento por sus credenciales falsas; binarios y archivos de mas de 1 MB no se escanean). Pendiente: el limite de 500 lineas de SKILL.md, solo tras completar G4 | 8 tests nuevos con repos temporales; `quality-gate.js --fast` cumple los 6 checks | Activar el check de SKILL.md al cerrar G4 |
