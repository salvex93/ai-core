...
| Cumplimiento del estandar de skills | Conforme en campos obligatorios; los 5 skills en o bajo 500 lineas (G4 cerrada); campos propios fuera de `metadata` | agentskills.io | Resuelta (G4); parcial (G5) |
...
5. G4 (RESUELTA, 2026-09-21: 5 SKILL.md divididos en nucleo + `references/`, los 5 en o bajo 500 lineas) y G16 (RESUELTA, 2026-09-21: limite de 500 lineas en SKILL.md activado en el gate). Pendiente: G5 (campos propios del frontmatter fuera de `metadata`; mayor esfuerzo, por skill con `npm run eval-skills` y `validate-globals` como red).
...
|---|---|---|---|---|
| 3 | G5 | RESTANTE tras G4/G16 (ambas RESUELTAS 2026-09-21): campos propios (`origin`, `version`, `last_updated`, `rol`) fuera de `metadata` en el frontmatter de los 45 skills | Alto | Migrar bajo `metadata` y ajustar `validate-globals`, `audit-market` y evals |
