...
| G6 | RESUELTA. Los 5 tests se dividieron por describe en 12 archivos (todos bajo 300 lineas; conteo de tests preservado; 1436 en total tras G17). El limite se hace cumplir ahora tambien en pre-push. | `quality-gate.js --fast` | Ninguna |
...
| G11 | Deadlock de investigacion: `web-search-guard.js` bloquea WebSearch/WebFetch si existe `GEMINI_API_KEY`, aunque el bridge no responda (429 de cuota o servidor no conectado). Ocurrio al intentar verificar G2 y G3. `guard-read.js` probablemente igual (no verificado) | sesion 2026-09-20: `buscar_web` devolvio 429 y WebFetch fue denegado | `mcp-gemini.js` escribe un marcador con TTL en `os.tmpdir()/ai-core-locks/` ante 429/RESOURCE_EXHAUSTED; ambos guards permiten la tool nativa mientras el marcador este vigente. Documentar en regla 7. Evaluar proveedor alterno para `buscar_web` (ModelRegistry) |
