...
- `ai-core-internal-governance`: reverificado contra el codigo del repo (sin fuente externa). `aiops-engineer` corrige el alcance de auditoria de 42 a 45 skills.
- Los siete skills de esos dominios suben `last_updated` a 2026-09-21 para que `audit-market` no los marque en DRIFT_VS_MERCADO.
...
`GeminiAdapter.js` solo tenia test del camino de timeout (`geminiadapter-js-timeout-colgado.test.js`); el camino exitoso de `chatGemini()` (mapeo de `usageMetadata`, fallback a 0 tokens, separacion de mensajes `system` vs `user`/`assistant`, override de `options.model`) no tenia cobertura. Agregado `tests/harness/geminiadapter-js-respuesta-exitosa.test.js` (5 casos), reutilizando el mismo patron de mock ya validado en el test de timeout: reemplazar `@google/genai` completo en `require.cache` por un `GoogleGenAI` de prueba, sin tocar el SDK real. `GeminiAdapter.js` paso a **100% linea / 100% rama / 100% funcion**.

`McpServerHandlers.js` (66.16% linea / 42.86% rama / 50% funcion) tenia una convencion deliberada en `tests/mcp-server-handlers.test.js` de no mockear `@google/genai` directamente, dejando sin cubrir el camino delegado/exitoso de sus 5 handlers (`analizarArchivo`, `analizarContenido` sin ningun test, `analizarRepositorio`, `resumirBacklog`, `buscarWeb`). Agregado `tests/harness/mcpserverhandlers-js-camino-delegado.test.js` (9 casos) mockeando `GeminiApiClient.js` — una capa propia del proyecto, no el SDK externo — mismo nivel de abstraccion que `CrossVerifier.js`/`SubagentGrader.js` mockean `ModelRegistry` en vez del SDK de cada proveedor. Igual que `SubagentGrader.js`, `McpServerHandlers.js` desestructura sus dependencias en el `require` (`const { getModel, isRefusal, ... } = require('./GeminiApiClient')`), asi que el mock invalida `require.cache` de ambos modulos en vez de mutar propiedades. `McpServerHandlers.js` paso a **98.48% linea / 79.31% rama / 100% funcion**.

Suite completa tras ambos cierres: 1405 tests, 1404 pass, 0 fail, 1 skip (esperado).

### Agregado — cobertura del camino "gh CLI disponible" en `issue-reporter.js`

`issue-reporter.js` (98.68% linea / 70.00% rama / 100% funcion antes de este cierre) solo tenia cobertura del camino "gh no disponible" (`issue-reporter-js.test.js`, PATH vacio) — el camino real de exito (autenticacion, listado de issues abiertos para deduplicar por titulo, creacion via `--body-file`, purga de eventos reportados con mas de 7 dias) no tenia ningun test, porque `gh` se invoca deliberadamente sin shell (`execFileSync`/`execSync` con array de argumentos, inmune a inyeccion de comandos — decision de seguridad documentada inline junto a `openIssue()`, no se toco).

Bloqueador real encontrado y resuelto: un fake `gh.cmd`/`gh.bat` antepuesto al `PATH` nunca se resuelve en Windows bajo Node v24 — `execFileSync`/`spawnSync` sin `shell:true` solo invocan binarios PE reales (`.exe`/`.com`) directamente, y priorizan el `gh.exe` real del sistema sin importar el orden del `PATH` (confirmado por experimentacion: `where.exe` si resuelve el fake primero, pero el resolver interno de Node no; invocar el `.cmd`/`.bat` por ruta absoluta produce `EINVAL`). `shell:true` si lo resolveria, pero no aplica porque `issue-reporter.js` no usa shell por diseño.

Agregado `tests/harness/issue-reporter-js-camino-gh-disponible.test.js` (7 casos) con un fake binario PE real: una copia de `process.execPath` (el propio `node.exe`) renombrada a `gh.exe`, cuyo comportamiento se controla via `NODE_OPTIONS=--require <hook>` — el `--require` intercepta antes de que Node intente resolver `argv[1]` como ruta de script. El hook reconstruye los argumentos reales concatenados en `argv[1]` (`cwd + primerArgumento`) y responde segun variables de entorno (`FAKE_GH_MODE`, `FAKE_GH_TITULOS_ABIERTOS`, `FAKE_GH_ULTIMO_BODY_FILE_OUT`). Como `NODE_OPTIONS` se hereda tambien al proceso principal que ejecuta `issue-reporter.js` (no solo al `gh.exe` fake que este invoca), el hook incluye un guard (`process.execPath` debe terminar en `gh.exe`) para no interceptar y terminar el propio script bajo prueba.

`issue-reporter.js` quedo en **98.68% linea / 70.00% rama / 100% funcion** (lineas 75-76 y 196-197 sin cubrir — ramas de error especificas de bajo riesgo, no bloqueantes para produccion). Suite completa: 1412 tests, 1411 pass, 0 fail, 1 skip (esperado).

### Agregado — test dedicado para `SubagentGrader.js` (cobertura de rama)

`SubagentGrader.js` (LLM-as-judge de output de subagentes, "Performance Outcomes" del Claude Agent SDK) no tenia ningun test propio — el test existente del hook `subagent-grader.js` solo ejercia inputs triviales via `spawnSync`, nunca las ramas reales de `calificar()`. Agregado `tests/harness/subagentgrader-js-calificacion-de-subagentes.test.js` (16 casos) cubriendo `calificar`, `parsearGrado`, `seleccionarJuez` y `construirPromptSistema`, incluyendo fallo cerrado (score 0) cuando el juez responde JSON no parseable.

Hallazgo real durante la implementacion: `SubagentGrader.js` desestructura `chat` de `ModelRegistry` en el `require` (`const { chat } = require('./ModelRegistry')`), a diferencia de `CrossVerifier.js` que usa `ModelRegistry.chat(...)` completo — el patron de mock ya validado (mutar `ModelRegistry.chat` en caliente) no alcanza aqui porque la referencia local queda fijada al valor original desde el momento del `require`. Dos de los 16 tests iniciales terminaron disparando llamadas de red reales (una fallo por falta de `DEEPSEEK_API_KEY`, la otra golpeo Gemini real con key valida en `.env` y devolvio un resultado distinto al esperado). Corregido invalidando `require.cache` de `ModelRegistry.js` y `SubagentGrader.js` antes de cada test mockeado, forzando que el modulo vuelva a desestructurar `chat` ya parcheado.

`SubagentGrader.js` paso de cobertura no medida a **100% linea / 94.29% rama / 100% funcion**. El hook `subagent-grader.js` (`.claude/bin/`) se mantiene en 33.33% rama/funcion — limite estructural: es un script standalone sin `module.exports`, ejecutado unicamente via `spawnSync`, por lo que sus ramas de decision (catch, proveedor null, nivel BAJO) solo son ejercitables ejerciendo la logica real de `SubagentGrader.js` (ya al 100%/94.29%/100%), no el wrapper del hook en si. Suite completa: 1391 tests, 1390 pass, 0 fail, 1 skip (esperado).

### Cerrado — gap de cobertura `--permission` real en `subagent-guard-release.js`

Auditoria amplia de agosto (`proyecto_auditoria_amplia_2026_08_28`) dejo pendiente confirmar si los 11 guards de seguridad originales seguian sin cobertura del Node.js Permission Model real (`spawnSync` sin `--permission` nunca reproduce el Permission Model, un guard puede pasar tests y seguir roto en produccion). Verificado en esta sesion: los 11 originales y varios agregados despues (tool-repeat, subagent-budget, loop-alternante, web-search) ya tenian cobertura completa en `tests/harness/sandbox-permission-smoke.test.js` desde el commit `99413c4`. El unico gap real encontrado: `subagent-guard-release.js` (invocado en produccion con `readYWriteSubagentLocks` via `hooks-definition.js`) tenia solo un test con `spawnSync` sin `--permission`. Agregado el par CON/SIN permiso al smoke test centralizado, verificando el comportamiento real: con el permiso correcto borra el lock y termina exit 0; sin permiso falla EPERM controlado, nunca cuelga ni lanza excepcion no capturada (compatible con el `2>/dev/null || true` con el que el hook ya lo envuelve en produccion). `node --test tests/harness/sandbox-permission-smoke.test.js` — 34/34 pass (2 nuevos). Suite completa 1374/1375 (1 skip), 45/45 skills conformes.
